      ...
      <section className="relative w-full bg-[#0b1f14] overflow-hidden">
        ...
        <div className="absolute inset-0">
          <Image
            ...
            sizes="100vw"
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#0b1f14]/85 via-[#0d2218]/92 to-[#0b1f14]" />
        </div>

        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 pt-28 pb-10 sm:pt-32 sm:pb-12">

          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-[11px] font-medium text-emerald-300/70 mb-5 flex-wrap">
            <Link href="/" className="hover:text-[#a3e635] transition-colors">Home</Link>
            <ChevronRight className="size-3 text-emerald-700 shrink-0" />
            <Link href="/blog" className="hover:text-[#a3e635] transition-colors">Blog & Insights</Link>
            <ChevronRight className="size-3 text-emerald-700 shrink-0" />
            <span className="text-emerald-400/80 truncate max-w-[180px] sm:max-w-xs">{article.category}</span>
          </nav>

          {/* Category + Meta */}
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="px-3 py-1 rounded-full text-[11px] font-black bg-[#84cc16] text-slate-950 uppercase tracking-wider">
              {article.category}
            </span>
            <span className="flex items-center gap-1.5 text-[11px] text-emerald-300/80 font-medium">
              <Clock className="size-3 text-[#a3e635]" />
              {article.readTime}
            </span>
            <span className="text-emerald-700">·</span>
            <span className="flex items-center gap-1.5 text-[11px] text-emerald-300/80 font-medium">
              <Calendar className="size-3 text-[#a3e635]" />
              {article.date}
            </span>
          </div>
          ...
          {/* Excerpt */}
          <p className="text-sm sm:text-base text-emerald-100/70 max-w-3xl leading-relaxed font-normal mb-6">
            {article.excerpt}
          </p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-5 border-t border-white/10">
            ...
            <div className="flex items-center gap-3">
              {article.authorImage && (
                <div className="relative size-11 rounded-full overflow-hidden border-2 border-[#84cc16] shrink-0">
              ...
              <div>
                ...
                <p className="text-[11px] text-emerald-400 font-medium">{article.authorRole || "Ufulu Finance"}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border text-[11px] font-bold transition-all cursor-pointer ${
                  liked
                    ? "bg-[#84cc16] text-slate-950 border-[#84cc16]"
              ...
              >
                {copied ? (
                  <><Check className="size-3.5 text-[#a3e635]" /><span className="text-[#a3e635]">Copied!</span></>
                ) : (
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-14">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 lg:gap-10 items-start">
          ...
          <article className="space-y-8 min-w-0">
            ...
            {article.keyTakeaways && article.keyTakeaways.length > 0 && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 space-y-4">
                <div className="flex items-center gap-2.5">
                  <div className="size-7 rounded-lg bg-[#84cc16] flex items-center justify-center shrink-0">
                    <Sparkles className="size-3.5 text-slate-950" />
                  </div>
                  <span className="text-xs font-black uppercase tracking-widest text-[#1b4332]">Key Takeaways</span>
                </div>
                ...
                if (numbered) {
                  ...
                  return (
                    <div key={i} className="flex gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm">
                      <div className="size-10 rounded-xl bg-[#1b4332] text-[#a3e635] font-black text-sm flex items-center justify-center shrink-0">
                        {num.padStart(2, "0")}
                      </div>
            ...
            {/* Pull Quote */}
            {article.quote && (
              <blockquote className="relative rounded-2xl bg-[#0f2819] text-white px-8 py-8 overflow-hidden">
                ...
                <p className="relative text-lg sm:text-xl font-medium italic leading-relaxed text-emerald-100 mb-4">
                  "{article.quote.text}"
                </p>
                <cite className="text-xs font-black uppercase tracking-wider text-[#a3e635] not-italic">
            ...
            {/* CTA Banner */}
            <div className="rounded-2xl bg-slate-950 border border-emerald-900/30 p-7 relative overflow-hidden">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_rgba(132,204,22,0.12),_transparent_65%)] pointer-events-none" />
              <div className="relative space-y-3">
                <span className="inline-block px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-[#84cc16] text-slate-950 mb-1">
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  <LoanEnquiryDialog
                    triggerButton={
                      <button
                        type="button"
                        className="inline-flex items-center gap-2 rounded-full bg-[#84cc16] hover:bg-[#a3e635] text-slate-950 font-bold px-6 py-2.5 text-xs tracking-wide uppercase transition-all cursor-pointer shadow-sm"
                      >
                        Apply for Financing
            ...
            <div className="flex gap-5 items-start bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm">
              {article.authorImage && (
                <div className="relative size-16 rounded-xl overflow-hidden border-2 border-[#84cc16] shrink-0">
              ...
              <div className="space-y-1.5">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-[#1b4332] text-[10px] font-bold">
                    <ShieldCheck className="size-2.5 text-[#65a30d]" />
                    Verified Contributor
                  </span>
                </div>
                <p className="text-[11px] font-semibold text-[#1b4332]">{article.authorRole || "Ufulu Finance"}</p>
          ...
          <aside className="space-y-5 lg:sticky lg:top-24">
            ...
            <div className="bg-[#0f2819] text-white rounded-2xl p-5 space-y-3.5 border border-emerald-900/40">
              <div className="flex items-center justify-between">
                <span className="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-[#84cc16] text-slate-950">
                  Loan Estimator
                </span>
                <Calculator className="size-4 text-[#a3e635]" />
              </div>
              ...
              <Link
                href="/loans"
                className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-[#84cc16] hover:bg-[#a3e635] text-slate-950 font-bold text-xs transition-colors"
              >
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              ...
              <div className="space-y-2.5">
                ...
                <div className="rounded-xl bg-slate-50 border border-slate-100 p-3.5 space-y-1.5">
                  ...
                  <a href="tel:+265991234567" className="flex items-center gap-1.5 text-[11px] text-[#1b4332] font-semibold hover:underline">
                ...
                <div className="rounded-xl bg-slate-50 border border-slate-100 p-3.5 space-y-1.5">
                  ...
                  <a href="tel:+265881234567" className="flex items-center gap-1.5 text-[11px] text-[#1b4332] font-semibold hover:underline">
                    <Phone className="size-3 shrink-0" />
                    +265 88 123 4567
                  </a>
                </div>
                ...
                <div className="rounded-xl bg-slate-50 border border-slate-100 p-3.5 space-y-1.5">
                  ...
                  <a href="mailto:info@ufulufinance.com" className="flex items-center gap-1.5 text-[11px] text-[#1b4332] font-semibold hover:underline">
              ...
              <Link
                href="/contact"
                className="flex items-center justify-center gap-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-[#1b4332] font-bold text-xs py-2.5 transition-colors"
              >
                View Full Directory
            ...
            <div className="bg-slate-900 text-white rounded-2xl p-5 space-y-3">
              <span className="text-[10px] font-black uppercase tracking-wider text-[#a3e635]">Stay Informed</span>
              ...
              {subscribed ? (
                <div className="rounded-xl bg-emerald-900/40 border border-emerald-700/40 py-3 text-center">
                  <p className="text-xs font-bold text-[#a3e635]">You're subscribed!</p>
                  <p className="text-[11px] text-emerald-300/70 mt-0.5">We'll be in touch soon.</p>
                </div>
              ) : (
                <form
                ...
                >
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl text-xs bg-white/8 border border-white/15 text-white placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-[#84cc16]"
                  />
                  <button
                    type="submit"
                    className="w-full rounded-xl bg-[#84cc16] hover:bg-[#a3e635] text-slate-950 font-bold text-xs py-2.5 transition-colors cursor-pointer flex items-center justify-center gap-2"
                  >
        ...
        {relatedArticles.length > 0 && (
          <section className="mt-16 pt-12 border-t border-slate-200">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
              <div>
                <p className="text-[11px] font-black uppercase tracking-widest text-[#65a30d] mb-1">Keep Reading</p>
                <h3 className="text-2xl font-extrabold text-slate-900">Related Insights & Market Updates</h3>
              </div>
              <Link
                href="/blog"
                className="inline-flex items-center gap-1.5 text-xs font-bold text-[#1b4332] hover:text-[#65a30d] transition-colors shrink-0"
              >
                View All Insights
                <ArrowRight className="size-3.5" />
              </Link>
              ...
              {relatedArticles.map((rel) => (
                ...
                >
                  ...
                  <div className="p-4 flex flex-col flex-1 gap-1.5">
                    ...
                    <h4 className="text-sm font-bold text-slate-900 group-hover:text-[#1b4332] transition-colors leading-snug line-clamp-2">
                      {rel.title}
                    </h4>
                    <p className="text-xs text-slate-500 leading-relaxed line-clamp-2 flex-1">
                      {rel.excerpt}
                    </p>
                    <div className="mt-3 flex items-center justify-between text-[11px] font-bold text-[#1b4332] group-hover:text-[#65a30d] transition-colors">
